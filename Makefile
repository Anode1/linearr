...
# made the checks run against a binary being relinked: the R comparison went
...
# check: all five checks. Run it before a commit.
check: ut cliut readme java r

# figs: redraw the two figures in doc/img. The lm() one is drawn from the same
# output r-check.sh prints, so it cannot claim an agreement the check does not
...
# clean for 300 rounds while the defect was live. The differential checks (java,
...
hooks:
	...
	@echo "hooks enabled: scripts/hooks/pre-push runs every check, then both sanitizers"
