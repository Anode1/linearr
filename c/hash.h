...
/* size is the initial bucket count; the table doubles as it fills. */
struct hash *hash_create(long size);
void         hash_delete(struct hash *table);              /* frees keys + table */

/* Insert or replace. Returns the PREVIOUS data for key, or NULL, so the caller
 * can free what it displaced. The table owns keys, never data. */
